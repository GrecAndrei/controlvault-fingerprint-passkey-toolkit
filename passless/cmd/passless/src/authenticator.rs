...
use crate::fingerprint::{FingerprintVerification, verify_fingerprint};
...
impl<S: CredentialStorage, P: PinStorage> AuthenticatorCallbacks for PasslessCallbacks<S, P> {
    ...
    fn request_uv(&self, info: &str, user: Option<&str>, rp: &str) -> Result<UvResult> {
        ...
        if self.security_config.fingerprint_verification {
            match verify_fingerprint(self.security_config.notification_timeout) {
                FingerprintVerification::Matched => {
                    info!("User verification via fingerprint: matched");
                    return Ok(UvResult::AcceptedWithUp);
                }
                FingerprintVerification::Rejected(reason) => {
                    warn!("User verification via fingerprint denied: {reason}");
                    return Ok(UvResult::Denied);
                }
            }
        }
...
mod tests {
    ...
    fn test_service_creation() {
        ...
        let security_config = SecurityConfig {
            ...
            fingerprint_verification: false,
