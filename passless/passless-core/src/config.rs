...
pub struct SecurityConfig {
    ...
    /// Require a successful fprintd verification for built-in user verification.
    ///
    /// When enabled, a failed, unavailable, or timed-out fingerprint check denies
    /// the passkey operation; it never falls back to notification approval.
    #[arg(
        long = "fingerprint-verification",
        env = "PASSLESS_FINGERPRINT_VERIFICATION",
        action = ArgAction::Set,
        require_equals = true,
        num_args = 0..=1,
        default_missing_value = "true"
    ...
    #[serde(default)]
    pub fingerprint_verification: bool,
